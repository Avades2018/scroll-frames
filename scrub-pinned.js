...
class ScrubPinned extends HTMLElement {
  connectedCallback() {
    ...
    // 16:9 box (width drives height) so it can pin to the top. Edges are
    // feathered to transparent on the left, top and bottom (not the right),
    // so the animation melts into your Wix section background.
    this.innerHTML = `
      <div class="ss-pin" style="position:sticky;top:0;width:100%;aspect-ratio:16 / 9;
    ...
    // Feather left, top and bottom to transparent — right edge stays hard.
    const horiz = `linear-gradient(to right, transparent 0, #000 ${FADE}, #000 100%)`;
    const vert  = `linear-gradient(to bottom, transparent 0, #000 ${FADE}, #000 calc(100% - ${FADE}), transparent 100%)`;
    const maskVal = `${horiz}, ${vert}`;
    ...
    const pinEl = this.querySelector(".ss-pin");

    function frameForScroll() {
      const rect = host.getBoundingClientRect();
      const vh   = window.innerHeight;
      const boxH = pinEl.offsetHeight || vh; // 16:9 box height (varies with width)
      // Pinned, start-on-entry: 0% as the box enters the bottom of the screen,
      // 100% exactly as the box releases from the top. Using boxH keeps the
      // finish aligned with the release at any screen width (narrow = shorter
      // box = longer pin, which this accounts for).
      let p = (vh - rect.top) / Math.max(1, host.offsetHeight + vh - boxH);
