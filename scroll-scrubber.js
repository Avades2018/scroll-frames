...
class ScrollScrubber extends HTMLElement {
  connectedCallback() {
    ...
    // visible at the bottom of the screen. Read live (inside frameForScroll)
    // so it works even if Wix applies the attribute after the element loads.
    //
    // pin: "true" (default) holds the canvas fixed on screen while a tall
    // runway scrolls past (the section "locks"). "false" lets the section
    // scroll normally and scrubs continuously as it passes through — no lock.
    ...
    function frameForScroll() {
      ...
      let p;
      if (!pinned || trigger === "enter") {
        // Continuous travel: 0% the instant the top edge enters the bottom of
        // the viewport, 100% as the bottom edge leaves the top of the viewport.
        // With pin off, this plays smoothly as the section scrolls past with
        // no locking; the scrub length equals the section's height plus one
        // screen of scrolling.
        p = (vh - rect.top) / Math.max(1, host.offsetHeight + vh);
      } else {
        // pinned + "pin" trigger: 0% when the section's top hits the top
