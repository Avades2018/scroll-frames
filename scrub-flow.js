...
class ScrubFlow extends HTMLElement {
  connectedCallback() {
    ...
    // The element fills its own box (width + height come from Wix), so you can
    // size it and set margins/padding per breakpoint in the editor. Height is
    // tied to the viewport (100vh) so it stays stable on resize.
    this.innerHTML = `
      <div class="ss-bleed" style="position:relative;width:100%;height:100vh;
    ...
    function draw(i) {
      ...
      let dw, dh, dx, dy;
      // "cover": fill the box, cropping overflow (keeps the full-bleed look).
